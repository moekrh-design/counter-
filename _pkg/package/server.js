...
app.get('/kiosk', (req,res)=>{
  ...
  const services = db.services.filter(s=> (s.group||'')!=='teacher' && serviceAvailableNow(s, workDate));
  const teacher_services = db.services.filter(s=> (s.group||'')==='teacher' && !!s.is_active);
  const step = (req.query.step || '').toString();
  res.render('kiosk', { services, teacher_services, workDate, lang, step, ui, branding });
});
